...
if #transcribe_paths == 0 then error("transcribe.py not found in runtimepath") end
...
local function wait_for_file(filename, max_attempts, interval)
  for _ = 1, max_attempts do
    if vim.fn.filereadable(filename) == 1 then return true end
...
local function process_local_recording(filename)
  ...
  local using_existing_model = false

  Job:new({
    command = "python",
    args = { transcribe_path, filename, model_cfg.model, model_cfg.path },
    on_stdout = function(_, data)
      if not data or data:match("^%s*$") then return true end
      return false
    end,
    on_stderr = function(_, data)
      if data and data:match("^DOWNLOAD_STATUS:") then
        local message = data:gsub("^DOWNLOAD_STATUS:", "")
        vim.schedule(function()
          if message:match("MODEL_ALREADY_DOWNLOADED") then
            api.debug_log("Using already downloaded model:", model_cfg.model)
            using_existing_model = true
            ui.start_transcribing_status() -- Reset to transcribing status
          elseif message:match("^DOWNLOADING_MODEL:") then
            download_detected = true
            local dl_model = message:gsub("^DOWNLOADING_MODEL:", "")
            ui.show_downloading_status(dl_model)
          elseif message:match("^DOWNLOADING_PROGRESS:") then
            if download_detected then
              local dl_model = message:gsub("^DOWNLOADING_PROGRESS:", "")
              ui.show_downloading_status(dl_model)
            end
          end
          if message:match("MODEL_DOWNLOAD_COMPLETE") then ui.start_transcribing_status() end
          api.debug_log("Download status:", message)
        end)
        return true
      end
      return false
    ...
    on_exit = function(j, return_val)
      vim.schedule(function()
        ...
        if return_val == 0 then
          ...
          ui.show_error_status(
            "Local transcription failed: " .. table.concat(j:stderr_result(), "\n")
          )
        end
      end)
    end,
...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command(
    "Vocal",
    M.transcribe,
    { desc = "Start or stop audio recording and transcribe" }
  )
...
function M.transcribe()
  if recording.is_recording() then
    ui.show_recording_status(false)
    ...
    recording.start_recording(
      ...
      function() ui.show_recording_status(true) end,
      function(err) ui.show_error_status("Recording error: " .. err) end
    )
  end
end
